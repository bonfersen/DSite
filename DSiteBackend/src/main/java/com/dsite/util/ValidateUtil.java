...
public class ValidateUtil {
	...
	public static Boolean isEmptyOrCero(String strVal) {
		if (isEmpty(strVal))
			return true;
		if (strVal.equals(DSiteCoreConstants.CERO_CADENA))
			return true;
		return false;
	}
	 ...
	 * 
	 * @param expresion
	 *            Indica si una cadena tiene el valor "obligatorio" indica que el valor del campo es Obligatorio, este
	 *            metodo evalua los demas casos se evaluan como expresiones regulares JAVA
	 * @param valorCampo
	...
	public static Boolean isNotValidField(String expresion, Object valorCampo) throws Exception {
		if (expresion.compareTo(DSiteCoreConstants.VALIDACION_CAMPO_OBLIGATORIO) == 0) {
	...
	public static Boolean isPlacaValidatePlaca(String expresion) throws Exception {
		...
		Pattern pat3 = Pattern.compile("(\\Â¿+|\\?+|\\Â°+|\\Â¬+|\\|+|\\!+|\\#+|\\$+|" + "\\%+|\\&+|\\+|\\=+|\\â€™+|\\Â¡+|\\++|\\*+|\\~+|\\[+|\\]"
				+ "+|\\{+|\\}+|\\^+|\\<+|\\>+|\\@+|\\_+|\\-)");
	 ...
	 * @param expresion
	 *            contiene un número.
	 * @return TRUE si expresion es un número, una cifra puede coincidir al menos 1 vez y como máximo 50 veces.
	 * @throws Exception
	 ...
	 * @return TRUE si la expresion pertecenece al alfabeto, incluye la letra ñ mayúscula y minúscula.
	 * @CaracteresAceptados Espacio en blanco, punto, guión y backslash.
	 * @throws Exception
	 *             FALSE si se encuentran los siguientes carácteres especiales !•$%&()=#@|~€¬'¡+{}º.
	 */
	 ...
	 * @return TRUE si la expresion es un número. throws Exception FALSE si es diferente a un número.
	 */
